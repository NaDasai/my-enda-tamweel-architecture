...
System_Boundary(application, "Loyalty Platform") {
    ...
    Container(backend, "Backend Service", "SpringBoot", "Core API handling points calculation, token operations, and user management")
    Container(jobQueue, "Job Queue", "Redis", "Asynchronous processing for daily batch operations")
...
The core SpringBoot application responsible for:
...
* Modular SpringBoot architecture for separation of concerns
...
* Batch token transfers to user accounts
