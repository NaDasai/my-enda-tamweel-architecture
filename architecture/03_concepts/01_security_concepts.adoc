...
* **Refresh Token Hashing**: Refresh tokens are hashed before storage in the database. Even if the database is compromised, tokens cannot be used directly.
...
|Key Derivation
|Encryption keys derived with unique salts per user
